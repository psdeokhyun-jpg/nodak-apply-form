import { reviewSummary, rateLimited, clientIp, json } from '../lib/airtable.js'
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (rateLimited(`reviews:${ip}`, MAX_PER_MIN)) {
    return json({ ok: false, message: '시도가 많습니다. 1분 후 다시 시도해주세요.' }, 429)
  }

  const expected = process.env.ADMIN_PASSWORD
  if (!expected) {
    console.error('[reviews] ADMIN_PASSWORD 미설정')
  ...
  try {
    return json(await reviewSummary())
  } catch (e) {
    console.error('[reviews]', e)
    return json({ ok: false, message: '후기를 불러오지 못했습니다.' }, 500)
  }
}
